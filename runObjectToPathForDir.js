...
if (!args[0]) {
  console.log(
    `no path provided for the directory containing the generated DXF files, defaulting to ${DEFAULT_GENERATED_DXF_DIR}`
  );
...
const spawnProcessAndAwaitCompletion = async (
  processToBeSpawned,
  processArguments
) =>
...
async function stepThroughFiles(files) {
  ...
  await spawnProcessAndAwaitCompletion(RUN_OBJECT_TO_PATH_FOR_FILE_LOCATION, [
    fullFilePath,
  ]);
  fs.copyFileSync(fullFilePath, processedDXFFilesDirName + fileName);
  ...
  if (files.length) {
    ...
    console.log(
      `you can view the processed files by opening the folder located at ${__dirname}\\${processedDXFFilesDirName}`
    );
  }
}
...
fs.readdir(generatedDXFDirectory, async (err, files) => {
  if (files.length === 0) {
    console.error(
      `ERROR: no files found in directory: ${generatedDXFDirectory}`
    );
    process.exit(1);
  }
